---
description: Aeowun source code organization — layers, target environments, dependency injection, and folder structure conventions. Reference when adding new modules, services, or contributions.
applyTo: src/vs/**
---

# Source Code Organization

Aeowun follows a strict layered architecture based on the foundation of the VS Code project.
...
3. **`editor`** — Text editor core (no `node` or `electron-*` dependencies)
4. **`workbench`** — Full Aeowun workbench, panels, views, and framework
...
- `vs/workbench/api` — `aeowun.d.ts` API provider
