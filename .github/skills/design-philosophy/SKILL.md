...
description: The Aeowun design philosophy — a shared Values→Principles→Moves vocabulary for reasoning about UI in design terms instead of raw pixels. Use when designing, building, reviewing, or giving feedback on any visual surface; when deciding a radius, spacing, type role, icon size, border, color, or motion; or when translating a "this feels off" observation into a concrete, principled fix.
---

# Aeowun Design Philosophy

This skill is the **canonical Aeowun design philosophy** — the single source of
...
These four values name what we want people to *feel* using Aeowun - they're the
...
the values Aeowun is structurally inclined to *lose*:

- **Calm** is the first casualty of an IDE. Aeowun is information-dense and lived
...
- **Consistent** has the largest surface area to defend. Aeowun is built by many
...
|------|--------|-------|------------|
| **Control** | 4px | `--aeowun-cornerRadius-small` | interactible elements - buttons, inputs, list rows, tabs |
| **Inner** | 6px | `--aeowun-cornerRadius-medium` | non-control containers sitting *inside* a surface |
| **Outer** | 8px | `--aeowun-cornerRadius-large` | floating / overlay surfaces - menus, hovers, dialogs, toasts |

Pills (radius ≈ half the height) are **fully round**
(`--aeowun-cornerRadius-circle`), not "a big radius."
...
The standard border/separator stroke is **1px** (`--aeowun-strokeThickness`).
...
Every color is a `--aeowun-*` theme token, so the UI tracks the active theme and
high-contrast modes.
...
> and menu items (see
> [magy-instructions.md](../../magy-instructions.md),
